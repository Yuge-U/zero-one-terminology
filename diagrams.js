...
window.MotionDiagram=(()=>{
 ...
 function mount(t){stop();step=0;active=plan(t);const el=document.getElementById('motionDiagram');if(!el)return;
  ...
  if(section&&!section.querySelector('.see-ai-notice')){const notice=document.createElement('p');notice.className='see-ai-notice';notice.textContent='AIによるイメージです。';el.before(notice);}
