...
test('speech starts synchronously, selects local English, resumes and exposes failures',()=>{
 ...
 const window={speechSynthesis:synth,SpeechSynthesisUtterance:function(text){this.text=text}};
 ...
 window.speak('Help side');assert.deepEqual(calls,['speak','resume']);assert.equal(utterance.voice,voice);assert.equal(utterance.volume,1);
