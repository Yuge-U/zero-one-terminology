...
- state-updatesの変更記録を読み込み、お気に入りの解除も含めて項目単位で統合する。
- CANVASと同じ小容量ファイルPUTを使用。差分をUUID別ファイルへ保存してからuser-data.dataを更新する。共有ファイルが古い状態で上書きされても差分から復元できる。アップロードセッションは実アカウントで400となったため使用しない。
- 409/412は最大4回、再読込・統合して再試行。その他のエラーや破損・未対応バージョンでは上書きを停止。
- BearerトークンはGraphの宛先だけへ送る。変更記録を自動削除しない。
...
**公開前に必要な実アカウント検証:** SPA URI登録後、同じアカウントの二端末で接続、登録・解除、クイズ履歴同期、同時更新、オフライン復帰を確認してください。AppFolder権限での小容量PUTと差分の再読込を、実アカウントで確認します。テストの成功を実OneDriveでの確認済みという意味では扱いません。
