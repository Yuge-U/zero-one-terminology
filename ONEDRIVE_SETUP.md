...
**実アカウントでの追加確認項目:** SPA URI登録後、同じアカウントの二端末で接続、登録・解除、クイズ履歴同期、同時更新、オフライン復帰を確認してください。AppFolder権限での小容量PUTと差分の再読込を、実アカウントで確認します。テストの成功を実OneDriveでの確認済みという意味では扱いません。
...
公開サイトの実アカウントで、初回同期、お気に入りの登録・解除の保存を確認。アップロードセッションの400エラーは小容量PUT＋変更記録方式で解消しました。確認用のお気に入りは解除済みです。クイズの実クラウド保存と物理的な別端末での同期は未確認（自動ブラウザテストでは模擬Graphで検証済み）。
